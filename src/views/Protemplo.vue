    ...
    <div class="container mx-auto mt-5">
      <div class="grid grid-rows-3 grid-flow-col gap-4">
        <div class="row-span-2 bg-white">
          <form action class="m-5" @submit.prevent="enviarProtemplo()">
            <div class="mb-5">
              <div class="flex justify-center">
                <img class="text-center" src="img/money.png" alt="Sunset in the mountains" />
              </div>
              <h5 class="text-center text-gray-600">REGISTRAR PROTEMPLO</h5>
              <hr />
            </div>
            ...
            <div class="w-full px-3">
              <label
                class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
                for="grid-password"
              >valor protemplo</label>
              <input
                class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-password"
                type="number"
                placeholder="$ 273.200"
                v-model="form.cantidad_protemplo"
                required
              />
            </div>
        ...
        <div class="row-span-2 col-span-2 bg-white">
          <div class="flex flex-col">
            <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
              ...
              >
                <p class="text-center font-semibold text-gray-500 mt-4">REGISTRO DE PROTEMPLO</p>
                <table class="min-w-full">
                  ...
                  <tbody class="bg-white">
                    <tr
                      :key="protemplo.id_protemplo"
                      v-for="protemplo in protemplos"
                    ...
                    >
                      ...
                      >{{ protemplo.nombre }} {{ protemplo.apellido1 }} {{ protemplo.apellido2 }}</td>
                      <td class="text-center text-sm leading-5 text-gray-900 align-middle">
                        <span
                          class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800"
                        >$ {{ format(protemplo.cantidad_protemplo) }}</span>
                      </td>
                      <td class="flex text-center">
                        <button
                          @click="eliminar(protemplo.id_protemplo)"
...
export default {
  ...
  methods: {
    llenarSelectorPersonas() {
      axios
        .get(keys.baseUrl + `personas-select-protemplo/${this.form.fecha}`)
    ...
    registrarProtemplo() {
      axios
        .post(keys.baseUrl + "registrar-protemplo", this.form)
        .then((response) => response.data)
        .then((resp) => {
    ...
    fetchProtemplo() {
      axios
        .get(keys.baseUrl + "protemplo")
        .then((response) => response.data)
        .then((resp) => {
          this.protemplos = resp;
          // eslint-disable-next-line no-console
          console.log(resp);
        });
    },
    ...
    editar(protemplo) {
      var exist = this.selectPersonas.find((item) => {
        return item.id == protemplo.id_persona;
      });
      if (exist == undefined) {
        this.selectPersonas.push({
          id: protemplo.id_persona,
          nombre: protemplo.nombre,
          apellido1: protemplo.apellido1,
          apellido2: protemplo.apellido2,
        });
      }
      setTimeout(() => {
        var option = document.getElementById("persona").children;
        for (var i = 0; i < option.length; i++) {
          if (option[i].value == protemplo.id_persona) {
      ...
      axios
        .get(keys.baseUrl + `editar-protemplo/${protemplo.id_protemplo}`)
        .then((response) => response.data)
        .then((resp) => {
          this.form.id = protemplo.id_protemplo;
          this.form.fecha = resp[0].fecha;
          this.form.cantidad_protemplo = resp[0].cantidad_protemplo;
        });
    ...
    actualizar() {
      axios
        .post(keys.baseUrl + "actualizar-protemplo", this.form)
        .then((response) => response.data)
        .then((resp) => {
          // eslint-disable-next-line no-console
          console.log(resp);
    ...
    eliminar(id) {
      ...
      if (confirmar) {
        axios
          .delete(keys.baseUrl + `eliminar-protemplo/${id}`)
          .then((response) => {
            // eslint-disable-next-line no-console
    ...
    format(int) {
      const number = new Intl.NumberFormat("de-DE").format(int);
