    ...
    <div class="container mx-auto mt-5">
      <div class="grid grid-rows-3 grid-flow-col gap-4">
        <div class="row-span-2 bg-white">
          <form action class="m-5" @submit.prevent="enviarOfrenda()">
            <div class="mb-5">
              <div class="flex justify-center">
                <img class="text-center" src="img/money.png" alt="Sunset in the mountains" />
              </div>
              <h5 class="text-center text-gray-600">REGISTRAR OFRENDAS</h5>
              <hr />
            </div>
        ...
        <div class="row-span-2 col-span-2 bg-white">
          <div class="flex flex-col">
            <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
              ...
              >
                <p class="text-center font-semibold text-gray-500 mt-4">REGISTRO DE OFRENDAS</p>
                <table class="min-w-full">
                  ...
                  <tbody class="bg-white">
                    ...
                    >
                      ...
                      >{{ ofrenda.nombre }} {{ ofrenda.apellido1 }} {{ ofrenda.apellido2 }}</td>
                      <td class="text-center text-sm leading-5 text-gray-900 align-middle">
                        <span
                          class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800"
                        >$ {{ format(ofrenda.cantidad_ofrenda) }}</span>
                      </td>
                      <td class="flex text-center">
                        <button
                          @click="eliminar(ofrenda.id_ofrenda)"
...
export default {
  ...
  methods: {
    llenarSelectorPersonas() {
      axios
        .get(keys.baseUrl + `personas-select-ofrendas/${this.form.fecha}`)
    ...
    registrarOfrenda() {
      axios
        .post(keys.baseUrl + "registrar-ofrendas", this.form)
        .then((response) => response.data)
        .then((resp) => {
    ...
    editar(ofrenda) {
      var exist = this.selectPersonas.find((item) => {
        return item.id == ofrenda.id_persona;
      });
      if (exist == undefined) {
        this.selectPersonas.push({
          id: ofrenda.id_persona,
          nombre: ofrenda.nombre,
          apellido1: ofrenda.apellido1,
          apellido2: ofrenda.apellido2,
        });
      }
      setTimeout(() => {
        var option = document.getElementById("persona").children;
        for (var i = 0; i < option.length; i++) {
          if (option[i].value == ofrenda.id_persona) {
      ...
      axios
        .get(keys.baseUrl + `editar-ofrendas/${ofrenda.id_ofrenda}`)
        .then((response) => response.data)
        .then((resp) => {
          this.form.id = ofrenda.id_ofrenda;
          this.form.fecha = resp[0].fecha;
          this.form.cantidad_ofrenda = resp[0].cantidad_ofrenda;
        });
    ...
    actualizar() {
      axios
        .post(keys.baseUrl + "actualizar-ofrendas", this.form)
        .then((response) => response.data)
        .then((resp) => {
          // eslint-disable-next-line no-console
          console.log(resp);
    ...
    eliminar(id) {
      ...
      if (confirmar) {
        axios
          .delete(keys.baseUrl + `eliminar-ofrendas/${id}`)
          .then((response) => {
            // eslint-disable-next-line no-console
